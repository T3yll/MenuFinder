...
const Home: React.FC = () => {
  ...
  return (
    <div>
      ...
      <section className="hero" style={{ paddingTop: '0', marginTop: '0', height: 'auto' }}>
        ...
        <div className="hero-content" style={{ paddingTop: '2rem' }}>
          <h1 className="hero-title" style={{ margin:0 }}>Découvrez les meilleurs de {userCity}</h1>
          <p className="hero-subtitle">Trouvez facilement les menus et restaurants qui vous correspondent selon vos envies et votre emplacement.</p>
          <form className="search-form" onSubmit={handleSearch} style={{ width: '100%' }}>
            <input
              ...
              style={{ width: 'calc(100% - (65px + 20px))' }}
            />
            <button type="submit" className="search-button" style={{ fontSize: '1.2rem', width: '61px', height: '61px', right: '-1px' }}>
          ...
          <div className="restaurant-grid">
            {filteredRestaurants.length > 0 ? (
                ...
                <div key={restaurant.id} className="restaurant-card">
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    ...
                    <p className="restaurant-price">
                      {restaurant.priceRange} · <span className="restaurant-rating">{restaurant.rating} <span style={{ color: '#FFD166' }}>★</span></span>
                    </p>
                    <span className="restaurant-category">{restaurant.category}</span>
                    <div style={{ marginTop: '1rem', textAlign: 'right' }}>
                      <Link to={`/restaurant/${restaurant.id}`} className="view-menu-link">Voir le menu →</Link>
                    </div>
                  </div>
                </div>
              ))
              ...
              <div className="text-center py-12">
                <p className="text-xl text-dark/70">Aucun restaurant trouvé dans cette catégorie</p>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="cta-section">
        <div className="section-container">
          ...
          <p className="cta-text">Rejoignez MenuFinder et faites découvrir votre établissement et vos spécialités à des milliers de clients potentiels.</p>
          <Link
